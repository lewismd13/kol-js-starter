...
/***/ "./src/afterAdventure.ts":
/*!*******************************!*\
  !*** ./src/afterAdventure.ts ***!
  \*******************************/
/*! namespace exports */
/*! exports [not provided] [maybe used in afterAdventure (runtime-defined)] */
...
var hoboParts = ["charred hobo boots", "creepy hobo skull", "frozen hobo eyeballs", "hobo crotch", "hobo skin", "stinking hobo guts"];

function main() {
  if ((0,kolmafia__WEBPACK_IMPORTED_MODULE_0__.myLocation)() === (0,libram__WEBPACK_IMPORTED_MODULE_1__.$location)(_templateObject || (_templateObject = _taggedTemplateLiteral(["Hobopolis Town Square"])))) {
    var oldPartCounts = libram__WEBPACK_IMPORTED_MODULE_2__.getCommaSeparated("_todayHoboParts").map(function (p) {
      return parseInt(p);
    });
    var totalOldParts = oldPartCounts.reduce(function (a, c) {
      return a + c;
    });
    var currentPartCounts = hoboParts.map(function (p) {
      return mySessionResults()[p];
    });
    var totalCurrentParts = currentPartCounts.reduce(function (a, c) {
      return a + c;
    });

    if (totalOldParts == totalCurrentParts) {
      throw "Missed getting a part!";
    }

    libram__WEBPACK_IMPORTED_MODULE_2__.set("_todayHoboParts", currentPartCounts.map(function (i) {
      return "".concat(i);
    }).join(","));
  }
}
...
/******/ 	return __webpack_require__("./src/afterAdventure.ts");
/******/ })()
